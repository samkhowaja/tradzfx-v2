...
//|    Add: http://127.0.0.1:3002  (TradeMentor V2 web app)          |
//| 3. Drag EA onto any chart                                        |
//| 4. API Key is pre-filled; verify it matches V2 env if changed    |
//| 5. EA will poll config from server, backfill, then sync live bars|
//+------------------------------------------------------------------+
#property copyright "TradeMentor AI"
#property version   "4.20"
#property description "Pushes M1 candles + executes trade signals + work queue commands from TradeMentor VPS (v4.1)"
#property strict
...
void OnTimer()
{
   ...
   // ── Work Queue: poll for server commands (priority-driven) ──
   if(TimeCurrent() - g_lastWorkQueuePoll > WORK_QUEUE_POLL_SEC)
   {
      PollWorkQueue();
   ...
   for(int i = 0; i < g_symbolCount; i++)
   {
      ...
      {
         bool ok = DoIncrementalSync(i, sym);
         if(ok)
            g_consecutiveErrors[i] = 0;
         else
            g_consecutiveErrors[i]++;
...
bool DoIncrementalSync(int symIdx, string symbol)
{
   ...
   // Filter to only bars strictly after cursor
   int validIdx = -1;
   for(int r = 0; r < copied; r++)
   {
      if(rates[r].time > fromTime) { validIdx = r; break; }
   }
   if(validIdx < 0) return true; // No new bars is not an error

   int pushCount = copied - validIdx;
   MqlRates pushRates[];
   ArrayResize(pushRates, pushCount);
   for(int r = 0; r < pushCount; r++)
      pushRates[r] = rates[validIdx + r];
   ...
   return (pushed == 0); // 0 = ok (empty), negative = error
}
...
void OnTick()
{
   if(!g_execEnabled || g_paused) return;

   // Throttle: only poll every g_execPollSec seconds
   datetime now = TimeCurrent();
