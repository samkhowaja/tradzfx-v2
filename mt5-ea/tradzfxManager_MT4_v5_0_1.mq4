//+------------------------------------------------------------------+
//|                                          tradzfxManager_MT4.mq4|
//|                        Single server-managed EA for MT4 terminals|
//+------------------------------------------------------------------+
...
long TMOpenMarket(string symbol, string side, double lots, double sl, double tp,
                  int slippagePoints, string comment, long magic)
{
...
long TMOpenLimit(string symbol, string side, double lots, double limitPrice,
                 ...
                 string tif, datetime expiration)
{
   bool isBuy = (side == "buy" || side == "BUY");
   ...
   if(ticket < 0)
   {
      TMLogError("OrderSend limit failed: " + IntegerToString(GetLastError()));
      return 0;
   }
   return ticket;
   #endif
...
string AutoRegister()
{
   ...
   if(StringLen(label) == 0)
      label = "MT4 " + broker + " " + IntegerToString(login);
...
void ExecuteSignalJson(string sigJson, bool paperMode)
{
   ...
   if(executionStrategy == "market")
   {
      if(maxDriftPips > 0 && entrySlipPips > maxDriftPips)
         reason = "Entry drift " + DoubleToString(entrySlipPips, 1) + " pips > max " + DoubleToString(maxDriftPips, 1);
      else if(minRR > 0 && effectiveRR < minRR)
         reason = "Effective R:R " + DoubleToString(effectiveRR, 2) + " < min " + DoubleToString(minRR, 2);
      else
         useMarket = true;
   }
   ...
   else
   {
      ...
      else if(g_cfg.allowLimitOrders)
      {
         ...
         reason = "Entry drift " + DoubleToString(entrySlipPips, 1) + " pips and limit orders disabled";
   }
   ...
   if(useMarket)
   {
      ...
      if(ticket <= 0) retcode = GetLastError();
      if(ticket > 0)
         fillPrice = TMCurrentPrice(symbol, isBuy);
   }
   else if(useLimit)
   {
...
void PollCommands()
{
   string url = g_serverUrl + "/api/mt5/commands";
   string body = TMHttpGet(url, g_effectiveApiKey, 10000);
   if(StringLen(body) == 0) return;
...
void ExecuteCommandJson(string cmdJson)
{
   ...
   {
      double lots = closeLots > 0 ? TMNormalizeLots(symbol, closeLots) : OrderLots((int)ticket);
...
//+------------------------------------------------------------------+
//| Helper: get order lot size for close                               |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Sync 1m candles for all symbols                                    |
//+------------------------------------------------------------------+
void SyncAllSymbols()
{
...
void SyncSymbol(int idx)
{
   ...
   datetime lastKnown = iTime(brokerSym, PERIOD_M1, 0) - 5 * 60;
...
//+------------------------------------------------------------------+
//| Fetch and push an exact [fromTime, toTime] window                  |
//+------------------------------------------------------------------+
...
string BuildBarsJson(string symbol, const MqlRates &rates[], int start, int end, string jobId = "")
{
   ...
   string sourceObj = "{\"platform\":\"mt4\",\"broker\":\"" + broker + "\",\"accountType\":\"" + acctType + "\",\"digits\":" + IntegerToString(digits);
