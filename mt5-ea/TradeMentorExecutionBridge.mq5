#define TM_EXEC_ONLY
#define TM_EA_NAME "TradeMentorExecutionBridge"
//+------------------------------------------------------------------+
//|                                           TradeMentorSync.mq5    |
//|                        TradeMentor AI -- MT5 VPS Sync EA         |
//|                                                                  |
//| PURPOSE:                                                         |
//| Push M1 candle history + live bars from MT5 to TradeMentor VPS   |
...
//| 4. Set your API Key (from TradeMentor AI -> MT5 Connector)       |
//| 5. EA will poll config from server, backfill, then sync live bars|
//+------------------------------------------------------------------+
#property copyright "TradeMentor AI"
#property version   "4.21"
#property description "Pushes M1 candles + executes trade signals + work queue commands from TradeMentor VPS (v4.21 � tick-triggered live sync)"
#property strict
...
input string InpApiKey       = "tm_mt5_93b214780ae6fdd83a726629535213b94e64bc3d4c0294ef"; // API Key (from TradeMentor VPS)
input string InpServerUrl    = "http://127.0.0.1:3002";                          // V2 Server URL (port 3002 — TradeMentor V2 web app)
...
input bool   InpExecEnabled  = true;                                  // Enable trade execution (signals -> orders)
input bool   InpExecPaperMode = true;                                  // Paper mode: log signals but don't execute (safe start)
...
#else
   #define EA_NAME         "TradeMentorSync"
#endif
...
bool     g_trackedPending[];      // True when tracked ticket is still a pending order
...
datetime g_lastSpreadPoll = 0;    // Last time we posted spread snapshots
...
int OnInit()
{
   ...
   ArrayResize(g_trackedPending, MAX_TRACKED);
   g_trackedCount = 0;
...
void OnTimer()
{
   ...
   if(TimeCurrent() - g_lastConfigPoll > configInterval)
   {
      PollCloudConfig();
   ...
   if(ModeExecEnabled() && TimeCurrent() - g_lastWorkQueuePoll > WORK_QUEUE_POLL_SEC)
   {
      PollWorkQueue();
      g_lastWorkQueuePoll = TimeCurrent();
   }

   // -- Trade History Sync (every 5 min) --
   if(ModeExecEnabled() && TimeCurrent() - g_lastTradeSync >= TRADE_SYNC_SEC)
   {
      SyncTradeHistory();
      g_lastTradeSync = TimeCurrent();
   }

   // -- Spread snapshot polling (every 60s) --
   if(ModeExecEnabled() && g_execEnabled && !g_paused && TimeCurrent() - g_lastSpreadPoll >= 60)
   {
      PollAndReportSpreads();
      g_lastSpreadPoll = TimeCurrent();
   }
...
bool CheckServerStatus(string symbol)
{
   ...
   {
      string body = CharArrayToString(result);
      Print(EA_NAME + ": Server returned HTTP ", res, ": ", body);
      return false;
   }
...
bool DoBackfill(int symIdx, string symbol)
{
   ...
   datetime targetStart = TimeCurrent() - g_backfillDays * 86400;
...
bool DoIncrementalSync(int symIdx, string symbol)
{
   ...
   if(fromTime == 0)
   {
      fromTime = TimeCurrent() - 3600;
   }

   // Get the most recent bars, then filter to only after cursor
   MqlRates rates[];
   int copied = CopyRates(symbol, PERIOD_M1, TimeCurrent(), g_batchSize, rates);
...
int PushBatch(string symbol, MqlRates &rates[], int count)
{
   ...
   long brokerOffsetSec = (long)(TimeCurrent() - TimeGMT());
   ...
   if(httpCode != 200)
   {
      ...
      Print(EA_NAME + ": Server HTTP ", httpCode, ": ", StringSubstr(body, 0, 200));
      return -1;
   }
   ...
   double entryZonePips = 0.0)
{
   ...
   // Signal reached the EA and passed local preprocessing.
   // ACK first so the timeout monitor can distinguish transport success from execution outcome.
   ReportSignalAck(signalId, 0);
   ...
   if(paperMode)
   {
      ...
      g_totalPaperLogged++;
      ReportSignalFilled(signalId, 0, paperPrice);
      return;
   }
   ...
   if(tradeResult.retcode == TRADE_RETCODE_DONE || tradeResult.retcode == TRADE_RETCODE_PLACED)
   {
      ...
      if(isLimit)
      {
         ...
         if(g_trackedCount < MAX_TRACKED)
         {
            ...
            g_trackedPending[g_trackedCount] = true;
            g_trackedCount++;
         }

         // ACK already sent above; actual fill will be reported when the pending order becomes a position.
      }
      else
      {
         ...
         if(g_trackedCount < MAX_TRACKED)
         {
            ...
            g_trackedPending[g_trackedCount] = false;
            g_trackedCount++;
         }

         g_totalOrdersFilled++;
         ReportSignalFilled(signalId, posId, fillPrice);
      }
   }
   else
   {
...
//+------------------------------------------------------------------+
//| ACK a received signal so the server can advance lifecycle state  |
//+------------------------------------------------------------------+
void ReportSignalAck(string signalId, long mt5Ticket)
{
   string json = "{";
   json += "\"signalId\":\"" + signalId + "\",";
   json += "\"ticketId\":" + IntegerToString(mt5Ticket) + ",";
   json += "\"timestamp_ms\":" + IntegerToString((int)(TimeCurrent() * 1000)) + ",";
   json += "\"acked_at\":\"" + TimeToString(TimeCurrent(), TIME_DATE | TIME_SECONDS) + "\"";
   json += "}";

   string url = InpServerUrl + "/api/mt5/signal-acked";
   string headers = "Content-Type: application/json\r\nX-API-Key: " + InpApiKey + "\r\n";
   ...
   if(httpCode != 200)
   {
      Print(EA_NAME + " [exec]: ACK report HTTP ", httpCode,
            " for signal ", signalId);
   }
}

//+------------------------------------------------------------------+
//| Report a real execution fill for slippage tracking               |
//+------------------------------------------------------------------+
void ReportSignalFilled(string signalId, long mt5Ticket, double actualPrice)
{
   string json = "{";
   json += "\"signalId\":\"" + signalId + "\",";
   json += "\"ticketId\":" + IntegerToString(mt5Ticket) + ",";
   json += "\"actualPrice\":" + DoubleToString(actualPrice, 5) + ",";
   json += "\"filledAtMs\":" + IntegerToString((int)(TimeCurrent() * 1000));
   json += "}";

   string url = InpServerUrl + "/api/mt5/signal-filled";
   string headers = "Content-Type: application/json\r\nX-API-Key: " + InpApiKey + "\r\n";
...
//+------------------------------------------------------------------+
//| Poll current spread for each active symbol and post snapshots    |
//+------------------------------------------------------------------+
void PollAndReportSpreads()
{
   ...
   for(int i = 0; i < g_symbolCount; i++)
   {
      ...
      string baseSym = (i < ArraySize(g_baseSymbols) && StringLen(g_baseSymbols[i]) > 0)
                       ? g_baseSymbols[i]
                       : brokerSym;
      ...
      double point = SymbolInfoDouble(brokerSym, SYMBOL_POINT);
      int digits = (int)SymbolInfoInteger(brokerSym, SYMBOL_DIGITS);
      int pipDigits = (digits == 3 || digits == 5) ? 10 : 1;
      double spreadPips = 0.0;
      if(point > 0.0)
         spreadPips = (ask - bid) / (point * pipDigits);

      string json = "{";
      json += "\"symbol\":\"" + baseSym + "\",";
      json += "\"spread_pips\":" + DoubleToString(spreadPips, 3) + ",";
      json += "\"bid\":" + DoubleToString(bid, digits) + ",";
      json += "\"ask\":" + DoubleToString(ask, digits) + ",";
      json += "\"timestamp_ms\":" + IntegerToString((int)(TimeCurrent() * 1000));
      json += "}";

      string url = InpServerUrl + "/api/mt5/spreads";
      string headers = "Content-Type: application/json\r\nX-API-Key: " + InpApiKey + "\r\n";
      ...
      if(httpCode != 200)
      {
         Print(EA_NAME + " [exec]: Spread report HTTP ", httpCode,
               " for symbol ", baseSym);
      }
   }
}
...
void MonitorOpenPositions()
{
   ...
   for(int i = g_trackedCount - 1; i >= 0; i--)
   {
      ...
      // Pending orders that have just turned into positions: report the actual fill once.
      if(i < ArraySize(g_trackedPending) && g_trackedPending[i] && isOpen)
      {
         double openPrice = PositionGetDouble(POSITION_PRICE_OPEN);
         Print(EA_NAME + " [exec]: PENDING FILLED — ticket=", ticket,
               " openPrice=", DoubleToString(openPrice, 5),
               " signal=", signalId);
         ReportSignalFilled(signalId, ticket, openPrice);
         g_totalOrdersFilled++;
         g_trackedPending[i] = false;
         continue;
      }
      ...
      if(!isOpen)
      {
         ...
         g_trackedPending[i] = g_trackedPending[g_trackedCount - 1];
         g_trackedCount--;
      }
   }
}
      ...
      if(cmdType == "INCREMENTAL")
      {
         ...
         if(StringLen(brokerSym) > 0)
         {
            ...
            else
            {
               ...
               MqlRates rates[];
               int copied = CopyRates(brokerSym, PERIOD_M1, TimeCurrent(), g_batchSize, rates);
               if(copied > 0)
               {
