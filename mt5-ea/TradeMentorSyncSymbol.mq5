...
#define TM_EA_NAME "TradeMentorSyncSymbol"
//+------------------------------------------------------------------+
//|                                           TradeMentorSync.mq5    |
...
datetime g_lastLiveSyncCheck = 0; // Last time DoLiveSyncPass ran (fallback timer)
datetime g_lastBarOpenTime[];   // Bar open time last pushed per symbol (candle-close gating)
uint     g_barNewOpenTick[];    // GetTickCount() when new bar was first detected per symbol
...
void RebuildSymbolList()
{
   if(ModeSingleSymbol())
   {
      ...
      ArrayResize(g_lastBarOpenTime, g_symbolCount);
      ArrayResize(g_barNewOpenTick, g_symbolCount);
      for(int i = 0; i < g_symbolCount; i++)
      {
         g_lastSyncTime[i] = 0;
         ...
         g_serverFirstMs[i] = 0;
         g_consecutiveErrors[i] = 0;
         g_lastBarOpenTime[i] = 0;
         g_barNewOpenTick[i] = 0;
      }
      return;
   }
   ...
   if(g_configSymbolCount > 0)
   {
      ...
      if(changed)
      {
         ...
         ArrayResize(g_lastBarOpenTime, g_configSymbolCount);
         ArrayResize(g_barNewOpenTick, g_configSymbolCount);
         ...
         for(int i = 0; i < g_configSymbolCount; i++)
         {
            ...
            if(StringLen(brokerSym) > 0)
            {
               ...
               g_lastBarOpenTime[g_symbolCount] = 0;
               g_barNewOpenTick[g_symbolCount] = 0;
