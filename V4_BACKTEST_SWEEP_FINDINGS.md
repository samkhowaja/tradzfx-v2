...
1. **Partition `features_zone` 5m** + partial index + retention (BUG-1)
2. **Deploy `refresh-lifecycle.js` on 15-min cron** for all active symbols (BUG-2)
3. **Add ATR to inline pipeline trigger** (`pipelineTrigger.ts`) (BUG-2)
4. **Create `candles_1d` hypertable** + backfill caggs (BUG-7)
5. **Complete `keylevel_bounce_v1` spec** or deactivate (BUG-3)

### P1 — High Impact
6. **Add `maxHoldBars` gate + spec validation** (BUG-4)
7. **Schedule `features_spread` refresh** + backfill (BUG-5)
8. **Verify/enable `features_order_block` @ 1m** (BUG-6)
9. **Partition `market_levels`** + retention (BUG-8)

### P2 — Medium Impact
10. **Backfill `features_correlation`** or remove correlation gates (BUG-9)
11. **Investigate `features_ifvg` sparsity** — producer bug or data gap? (BUG-10)
12. **Audit `watukushay_no1` regimeRelax logic** (BUG-13)
...
```bash
# 1. After partitioning features_zone 5m:
node scripts/backtest-pit-v2.js XAUUSD 90 doyle_sd --json
# Expect: queryMs < 500 (was 6442)

# 2. After refresh-lifecycle cron deployed:
node scripts/check-feature-freshness.js XAUUSD
# Expect: features_atr 1m < 30 min stale

# 3. After candles_1d created:
node scripts/backtest-pit-v2.js XAUUSD 90 watukushay_no1 --json
# Expect: no "table candles_1d does not exist" errors

# 4. After keylevel_bounce_v1 spec completed:
node scripts/backtest-pit-v2.js XAUUSD 90 keylevel_bounce_v1 --json
# Expect: executed > 0

# 5. After maxHoldBars gate added:
node scripts/backtest-pit-v2.js XAUUSD 90 smart_risk_ob_ifvg_1m_sniper_10r_demand_supply_zone_tp --json
# Expect: avgHoldBars << timeoutBars (not 221/480)
```

---

## 📝 NOTES FOR STRATEGY TEAM (Post-Skeleton-Fix)

Once P0-P1 fixes are deployed, re-run full sweep. Expected changes:
- `doyle_sd`: Query time 6.4s → <500ms; WR may improve with fresh ATR
- `a_plus_orb_fvg_5m`: Will run (no timeout); evaluate WR
- `watukushay_no1`: Fresh ATR + spread data → vol/spread gates accurate
- `sniper_10r`: `maxHoldBars` forces realistic hold times
- `keylevel_bounce_v1`: Will execute (spec permitting)

**Do not tune strategy parameters until skeleton is solid.** Current WR/R metrics are contaminated by stale features, timeouts, and empty tables.
...
|-----------|------|-----------|
| PIT SQL Compiler | `scripts/backtest-pit-v2.js` | `compilePITSQL()` ~881 |
| Trade Simulation | `scripts/backtest-pit-v2.js` | `simulateTrade()` ~1125 |
| Gate Application | `scripts/backtest-pit-v2.js` | `applyGates()` ~2275 |
| Setup Engine Batch | `packages/setupEngine/src/evaluateSetup.ts` | `evaluateSetupBatch()` ~292 |
| Persistence | `scripts/backtest-pit-v2.js` | `INSERT INTO backtest_results` ~2497 |
| Lifecycle Refresh | `scripts/refresh-lifecycle.js` | Main entry |
| Inline Pipeline | `apps/web/src/lib/pipelineTrigger.ts` | `runFeatureEngine()` |
| Spec Validation | `scripts/seed-strategy-specs.js` | `validateSpec()` |
| Feature Freshness Check | `scripts/check-feature-freshness.js` | (create if missing) |