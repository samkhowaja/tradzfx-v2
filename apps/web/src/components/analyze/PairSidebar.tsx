  ...
  return (
    ...
    >
      ...
      <span className="ml-auto flex shrink-0 items-center gap-1">
        ...
        <IconCaution
          dim={!pair.isStale && pair.bias !== null}
          title={
            pair.isStale
              ? "Caution: stale candle data"
              : pair.bias
              ? "Data fresh and bias available"
              : "Caution: no bias computed yet"
          }
        />

        {/* Clock — setup age (active signals) */}
        <IconClock
          dim={pair.activeSignals === 0}
          title={pair.activeSignals > 0 ? "Active setup(s)" : "No active setups"}
        />

        {/* Check — data fresh */}
        <IconCheck
          dim={pair.isStale}
          title={pair.isStale ? "Candle data is stale" : "Candle data is fresh"}
        />

        {/* Play — live feed */}
        <IconPlay
          dim={pair.isStale}
          title={pair.isStale ? "Live feed paused" : "Live feed active"}
        />
...
function IconCaution({ dim, title }: { dim: boolean; title?: string }) {
  return (
    <svg
      viewBox="0 0 16 16"
...
function IconClock({ dim, title }: { dim: boolean; title?: string }) {
  return (
    <svg
      viewBox="0 0 16 16"
...
function IconCheck({ dim, title }: { dim: boolean; title?: string }) {
  return (
    <svg
      viewBox="0 0 16 16"
...
function IconPlay({ dim, title }: { dim: boolean; title?: string }) {
  return (
    <svg
      viewBox="0 0 16 16"
...
function GradeBadge({ confidence }: { confidence: number | null }) {
  ...
  return (
    <span
      className={`text-[10px] font-bold leading-none ${tone}`}
      title={`Bias confidence: ${confidence}%`}
    >
      {grade}
    </span>
  );
