...
- Migration `146` aligns mutable SQL maintenance with replay semantics: cumulative maximum fill, first 50% penetration mitigation with invalidation fallback, close-beyond invalidation, canonical candles, and exact legacy identity. It removes formation-checkpoint skipping while suppressing unchanged writes. Shared engine lifecycle now computes cumulative maximum fill; order-block engine version advances to `1.4.1` for cache invalidation.
- After SQL alignment and bounded refresh, comparison improved from 218 to 94 lifecycle mismatches and from 164 to 76 fill mismatches across 315 identified events. Replay retained zero missing events and zero temporal-integrity failures. Remaining drift is attributable to active pre-deployment producer rewrites and bounded engine input; deploy `1.4.1`, refresh, and repeat comparison before any reader decision.
- Reader cutover remains blocked pending post-deployment parity evidence.
