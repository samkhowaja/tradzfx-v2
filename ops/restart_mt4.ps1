...
<#
.SYNOPSIS
    Restarts the MetaTrader 4 terminal(s) gracefully.
.DESCRIPTION
    - Detects running terminal processes and tries a graceful close first.
    - Falls back to force-kill after a timeout.
    - Restarts each stopped terminal from its original path (or $env:MT4_TERMINAL_PATH).
    - Verifies the process respawned.
#>
param(
    [string]$TerminalPath = $env:MT4_TERMINAL_PATH,
    [int]$GracefulTimeoutSec = 10,
    [int]$VerifyTimeoutSec = 30
)

$DefaultPath = "C:\Program Files (x86)\MetaTrader 4\terminal.exe"
...
$processes = Get-MtProcesses "terminal" "*MetaTrader 4*"
$pathsToStart = @()
...
if ($processes) {
    foreach ($proc in $processes) {
        ...
        $pathsToStart += $path
        Write-Host "Stopping MT4 terminal PID $($proc.Id) at $path"
        Stop-Terminal $proc $GracefulTimeoutSec | Out-Null
    }
} else {
    Write-Host "No running MT4 terminal found."
    $pathsToStart += (if ($TerminalPath) { $TerminalPath } else { $DefaultPath })
}

Write-Host "Starting MT4 terminal(s)..."
$started = @()
...
if (-not $started) {
    Write-Error "Failed to start any MT4 terminal."
    exit 1
}
...
while ($sw.Elapsed.TotalSeconds -lt $VerifyTimeoutSec) {
    $running = Get-MtProcesses "terminal" "*MetaTrader 4*"
...
if ($verified) {
    Write-Host "MT4 restart verified. Running terminals: $(($running | Select-Object -ExpandProperty Id) -join ', ')"
    exit 0
} else {
    Write-Error "MT4 restart verification failed."
    exit 1
}
