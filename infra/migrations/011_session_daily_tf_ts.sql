...
-- Drop and recreate primary keys to include tf (idempotent: only if the target PK does not already exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = 'features_opening_range'::regclass
      AND conname = 'features_opening_range_pkey'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = 'features_session_hl'::regclass
      AND conname = 'features_session_hl_pkey'
  ) THEN
