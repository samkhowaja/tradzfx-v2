-- Optimize iFVG lifecycle refresh using early-exit LATERAL index lookups.
-- iFVG has far more open rows than the other lifecycle tables, so the full
-- scan-to-as-of join in the previous version is too slow for batched backfills.
